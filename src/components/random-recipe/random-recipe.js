...
export default class RandomRecipe extends Component {
  ...
  updateRecipe = () => {
    ...
    this.healthyFoodService
      .getHealthyRecipe("healthy", randomRecipeNumber)
...
const RecipeView = ({ recipe, quantity, unit, onItemSelected }) => {
  ...
  return (
    <React.Fragment>
      ...
      <div>
        <span style={{ color: "#b5f341" }}>May be you would try this? ;)</span>
        <h4>{title}</h4>
        <ul className="list-group list-group-flush">
          <li className="list-group-item">
            <span className="term">Calories -</span>
            <span>
              {Math.round((calories * quantity) / totalWeight)} / {quantity}{" "}
              {unit}
            </span>
          </li>
          <li className="list-group-item">
          ...
          <li className="list-group-item">
            <button
              onClick={() => onItemSelected(id, query)}
